...
El test crea una base aleatoria, aplica DDL 002/003/016, usa el repositorio mediante
...
`python3 scripts/event-state-deploy.py` construye la imagen Docker, conserva la
...
No cambia volúmenes ni restaura datos automáticamente.
...
`evidence/os-05-ess/`, excluida de Git. Los PKC y referencias originales viven
...
## Incremento de ciclo de vida en desarrollo

La implementación de solicitudes explícitas OPEN/CLOSE/reapertura está descrita
 en [el contrato](lifecycle-contract.md). La adaptación de pruebas está pausada
por indicación del usuario durante la reorganización de `testing/`.
Véase [estado del incremento](lifecycle-status.md): el PASS de la baseline no
certifica todavía estas capacidades nuevas.
