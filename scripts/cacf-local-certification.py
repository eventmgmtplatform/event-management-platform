...
COMPOSE = ["docker", "compose", "-p", "cacf-certification", "-f", str(ROOT / "infrastructure/docker-compose.cacf-test.yml")]
...
def request(timeout=600, ticket=True):
    data = json.loads((ROOT / "services/integration-worker/src/test/resources/cacf/request.json").read_text())
