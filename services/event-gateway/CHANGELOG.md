...
Historia reconstruida desde Git local: fechas de autor y SHA verificables; no equivalen a releases o despliegues.
...
- API administrativa de reglas en el gateway: catálogo persistente, historial,
  ...
  preservando el original. Migración 026 y activación explícita por configuración.

- Recepción original persistida antes de validar o publicar; migración 025,
  usuario JDBC dedicado y endpoint de disponibilidad del almacenamiento.
- Despliegue local reproducible mediante `scripts/gateway-rules-deploy.py`,
  configuración privada excluida de Git y reversión de imagen/configuración si
  falla la activación. El catálogo existente se conserva.
- Pruebas del motor, autorización, simulación, revisión concurrente, historial
  y rollback transaccional; certificación HTTP reproducible contra PostgreSQL aislado.
- [Contrato y operación de la API](../../docs/event-gateway/rules-api.md).
