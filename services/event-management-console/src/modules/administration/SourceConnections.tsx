...
type Source = { id: "runtime" | "tickets" | "glpi"; health: "healthy" | "error"; endpoint: string; target: string };
export function SourceConnections() {
  ...
  useEffect(() => {
    ...
    fetch("/api/administration/sources", { signal: controller.signal, cache: "no-store" }).then(async response => {
      ...
      if (!Array.isArray(data.sources) || data.sources.length !== 3 || new Set(data.sources.map((s: Source) => s?.id)).size !== 3 || !data.sources.every((s: Source) => s && ["runtime", "tickets", "glpi"].includes(s.id) && ["healthy", "error"].includes(s.health) && typeof s.endpoint === "string" && typeof s.target === "string")) throw new Error();
