export type Rule={id:string;version:number;type:'SCHEDULED'|'IMMEDIATE';enabled:boolean;scope:Record<string,string>;schedule:{timezone:string;validFrom:string;validTo?:string|null};priority:number;reason:string;metadata:{owner:string;externalReference?:string|null}};
...
export function validateRule(rule:Rule,tenant:string){
 ...
 if(!['SCHEDULED','IMMEDIATE'].includes(rule.type)||rule.scope.customerCode!==tenant)throw new Error('La regla debe pertenecer al tenant seleccionado.');
 ...
 if(rule.type==='SCHEDULED'&&!end||end&&Date.parse(start)>=Date.parse(end))throw new Error('La ventana requiere inicio anterior al fin.');
...
export function operation(tenant:string,path:string,body:unknown,etag:string,ruleId:string):Operation{return {tenant,actor:'local-console',path,body:JSON.stringify(body),etag,key:crypto.randomUUID(),ruleId};}
