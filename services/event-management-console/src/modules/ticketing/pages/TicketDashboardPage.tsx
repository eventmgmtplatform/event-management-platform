...
import { snowTicketingRepository as repository } from "../services/snow-ticketing.adapter";
...
export function TicketDashboardPage() {
  ...
  async function load(next: TicketQuery) {
    ...
    try {
      const all = repository.search({ number: "", status: "All" }, current.signal);
      const filtered = next.number || next.status !== "All" ? repository.search(next, current.signal) : all;
      const [items, inventory] = await Promise.all([filtered, all]);
      if (controller.current !== current) return;
      setTickets(items); setTotals(inventory); setPage(1); setObserved(new Date().toISOString());
    } catch (reason) {
      if (controller.current !== current) return;
      setTickets([]); setTotals([]); setObserved("");
  ...
  async function close() {
    ...
    try {
      ...
      setSelected(updated); setClosing(false); setNotice(`${updated.id}: ${t("Cierre guardado en ServiceNow mock.")}`);
  ...
  return <section className="page ticket-page">
    <div className="page-heading"><div><span className="eyebrow">{t("Plugin Ticketing")}</span><h1>{t("Dashboard de tickets")}</h1><p>{t("Consulta y cierre por API · ServiceNow mock exclusivo de la consola.")}</p></div><span className="mock-chip">{t("Fuente: ServiceNow mock aislado")}</span></div>
    <div className="ticket-kpis" aria-label={t("Filtrar por estado")}>{(["All", ...ticketStatuses] as const).map(status => <button type="button" key={status} className={query.status === status ? "selected" : ""} aria-pressed={query.status === status} disabled={loading || saving} onClick={() => search({ ...query, status })}><small>{t(labels[status])}</small><strong>{error ? "—" : status === "All" ? totals.length : totals.filter(ticket => ticket.status === status).length}</strong></button>)}</div>
    <form className="ticket-filters" onSubmit={event => { event.preventDefault(); search({ ...query, number: input.trim() }); }}><label><span>{t("Buscar ticket")}</span><input aria-label={t("Buscar ticket")} value={input} onChange={event => setInput(event.target.value)} placeholder="INC0019284" maxLength={40} pattern="[a-zA-Z0-9-]*" title={t("Escribe el número completo o una parte del ticket.")}/></label><button type="submit" disabled={loading || saving}>{t("Buscar")}</button><button type="button" disabled={loading || saving} onClick={() => void load(query)}>{t("↻ Refresh")}</button></form>
    ...
    <div className="table-wrap" aria-busy={loading}><table><thead><tr>{["Ticket", "Estado", "Prioridad", "Cliente", "Recurso", "Resumen", "Torre", "Actualizado", "Detalle"].map(label => <th key={label}>{t(label)}</th>)}</tr></thead><tbody>{rows.map(ticket => <tr key={ticket.id} onDoubleClick={() => open(ticket)} onContextMenu={event => { event.preventDefault(); open(ticket); }}><td><button className="ticket-link" onClick={() => open(ticket)}>{ticket.id}</button></td><td><span className={`badge ticket-status-${ticket.status.replaceAll(" ", "-")}`}>{t(statusLabels[ticket.status])}</span></td><td>{t(ticket.priority)}</td><td>{ticket.customer}</td><td>{ticket.resource}</td><td title={ticket.summary}>{ticket.summary}</td><td>{ticket.tower}</td><td>{new Date(ticket.updated).toLocaleString(locale)}</td><td><button className="row-action" onClick={() => open(ticket)} aria-label={`${t("Ver detalle de")} ${ticket.id}`}>•••</button></td></tr>)}</tbody></table>{!rows.length && <div className="no-results">{t(loading ? "Consultando…" : error ? "Sin datos disponibles" : "Sin resultados")}</div>}</div>
