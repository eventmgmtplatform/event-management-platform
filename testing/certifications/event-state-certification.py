...
def drained(topic="integration.results", group="event-state-service"):
    ...
    return offsets_drained(output, topic, group)


def offsets_drained(output, topic, group):
    rows = [line.split() for line in output.splitlines()]
    rows = [row for row in rows if len(row) >= 6 and row[0] == group and row[1] == topic]
    # Kafka has no committed offset for a never-used partition. It is empty only
    # when the log end is exactly zero; unknown lag elsewhere must fail closed.
    return bool(rows) and all(row[5] == '0' or
                             (row[3], row[4], row[5]) == ('-', '0', '-') for row in rows)


def preflight(verify=False):
    # Inspect prerequisites before generating fixtures or touching mounted build output.
    tables = sql("SELECT count(*) FROM information_schema.tables WHERE table_schema='event_management' "
                 "AND table_name IN ('ess_quarantine','ess_state_request','ess_event_transition')")
    if tables != '3':
        raise AssertionError('ESS_LIFECYCLE_MIGRATIONS_REQUIRED: apply coordinated 016/017 deployment before certification')
    groups = run(COMPOSE + ['exec', '-T', 'kafka', '/opt/kafka/bin/kafka-consumer-groups.sh',
                 '--bootstrap-server', 'kafka:29092', '--list'])
    if 'event-state-service-lifecycle' not in groups.splitlines():
        raise AssertionError('ESS_LIFECYCLE_CONSUMER_REQUIRED: lifecycle consumer group is absent; inspect coordinated shared deployment')
    if verify:
        containers = run(['docker', 'ps', '-q']).splitlines()
        if containers:
            inspected = json.loads(run(['docker', 'inspect', *containers]))
            target = (ROOT / 'services/event-state-service/target').resolve()
            for container in inspected:
                for mount in container.get('Mounts', []):
                    source = Path(mount.get('Source', '/')).resolve()
                    if source == target or target in source.parents or source in target.parents:
                        raise AssertionError('ESS_BUILD_MOUNTED: stop the laboratory through lifecycle-prepare before Maven verification')
...
def main():
    ...
    try:
        ...
        spec=importlib.util.spec_from_file_location('ess_admin',ROOT/'scripts/event-state-admin.py')
        admin_client=importlib.util.module_from_spec(spec);spec.loader.exec_module(admin_client)
        page=admin_client.request('events')
        if 'items' not in page: raise AssertionError('ADMIN_API_INVALID_RESPONSE')
        try:
            admin_client.request('events',token_override='invalid')
            raise AssertionError('ADMIN_API_ACCEPTED_INVALID_TOKEN')
        except urllib.error.HTTPError as error:
            if error.code!=401: raise
        admin_client.request('quarantine')
        report['checks'].append('admin-api')
