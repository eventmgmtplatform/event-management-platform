...
def simulate():
    ...
    fixture_dir = ROOT / 'testing/fixtures/events/sdc'
...
def main():
    output = ROOT / 'evidences/testing/lifecycle' / datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')
    output.mkdir(parents=True)
    report = {'status': 'RUNNING', 'checks': [], 'limits': [
        'ServiceNow ticket routing exists; automatic ticket → GNM → CACF → clear chaining is not certified (UC-001).',
    ...
    try:
        ...
        run(['python3', ROOT / 'testing/certifications/cacf-local-certification.py'])
