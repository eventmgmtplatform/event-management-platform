...
def run(output,report,checkpoint=None):
    ...
    api='http://127.0.0.1:28082/api/v1';worker='http://127.0.0.1:28083'
    headers={'X-Tenant-Id':tenant,'X-Actor-Id':'os11-testing'}
    mocks={'sn':'http://127.0.0.1:28181','gnm':'http://127.0.0.1:28182','next':'http://127.0.0.1:28183'}
    ...
    def lifecycle():
        raw=sql("SELECT document::text FROM event_processor.lifecycle WHERE tenant='"+tenant+"' AND document->'initialCommand'->'payload'->>'resource'='"+node+"'")
    ...
    def send(recovery=False):
        ...
        status,accepted,_=http('POST','http://127.0.0.1:28081/api/v1/events',event);require(status==202 and accepted['accepted'],'Gateway rejected input')
    ...
    try:
        ...
        fatal=send();d=stage('ticket');require(d['ticketNumber']==ticket and d['ticketId']==sysid,'Ticket response identity mismatch')
        ...
        wait(lambda: sql("SELECT state FROM event_management.automation_execution WHERE execution_id='"+execution+"'")=='SUBMITTED','NEXT CREATE submitted',120)
        if checkpoint:checkpoint('NEXT_SUBMITTED',dict(report['identities']))
        callback(execution,'Acknowledge_Create')
        wait(lambda: sql("SELECT status FROM event_management.automation_provider_dispatch WHERE execution_id='"+execution+"' AND operation='TKTUPDATE'")=='SENT','ticket association',120)
        ...
        recovery=send(True);d=stage('COMPLETED')
        ...
        wait(lambda: sql("SELECT count(*) FROM event_processor.processing_record WHERE tenant='"+tenant+"' AND event_id='"+duplicate['eventId']+"'")=='1','duplicate recovery processed')
        def projected():
            raw=sql("SELECT row_to_json(s)::text FROM event_management.event_state s WHERE event_key='correlation:"+d['cycleId']+"'")
        ...
        def source_closed():
            raw=sql("SELECT row_to_json(s)::text FROM event_management.event_state s WHERE event_key='"+d['sourceEventKey']+"'")
            if not raw:return None
        ...
        def search_confirmed():
            try:
                group=http('GET','http://127.0.0.1:28092/events-current/_doc/'+urllib.parse.quote('correlation:'+d['cycleId'],safe=''))[1]['_source']
                src=http('GET','http://127.0.0.1:28092/events-current/_doc/'+urllib.parse.quote(d['sourceEventKey'],safe=''))[1]['_source']
        ...
        wait(lambda: sql("SELECT count(*) FROM event_processor.output_outbox WHERE processing_id='"+d['processingId']+"' AND published_at IS NULL")=='0','outbox drained')
        ...
        require(sql("SELECT count(*) FROM event_management.automation_outbox WHERE execution_id='"+execution+"' AND NOT published")=='0','CACF outbox pending')
        require(sql("SELECT count(*) FROM event_management.automation_provider_dispatch WHERE execution_id='"+execution+"' AND status<>'SENT'")=='0','Provider dispatch pending')
        wait(lambda: sql("SELECT count(*) FROM event_management.integration_command_execution WHERE event_id='"+d['cycleId']+"' AND (execution_status<>'COMPLETED' OR result_published_at IS NULL)")=='0','worker delivery drained',60)
