...
def main():
    ...
    try:
        if args.suite == 'happy-path':
            ...
            cacf = module(ROOT/'testing/certifications/cacf-local-certification.py')
            data = cacf.request()
            report.update(executionId=data['executionId'], scope='CACF component; isolated mock providers; not UC-001 E2E')
            cacf.submit(data)
            cacf.callback(data, 'Acknowledge_Create')
            cacf.wait(lambda: len(cacf.provider_calls(data, '/tupix/api/v1/netcool/incidents')) == 1, 'ticket association')
            cacf.callback(data, 'result', 'RESOLVE')
            cacf.wait(lambda: cacf.state(data).get('outcome') == 'REMEDIATED', 'successful automation')
            cacf.callback(data, 'result', 'RESOLVE')
            cacf.wait(lambda: len(cacf.ticket_calls(data)) >= 1, 'ITSM action')
            state = cacf.state(data)
            if state['state'] != 'COMPLETED' or state['outcome'] != 'REMEDIATED':
                raise AssertionError('Terminal success not stable after duplicate callback')
            (output/'automation.json').write_text(json.dumps(state, indent=2)+'\n')
            report['status'], code = 'PASS', 0
        else:
            report['commands'] = []
            commands = []
